...
# 50 "MCAL_Layer/Timers/hal_timer0.h"
typedef enum {
    PRESCALER_DIV_BY_2,
...
void (* Timer0_InterruptHandler) (void) = ((void*)0);
...
STD_ReturnType timer0_init(const timer0_t *timer) {
    ...
    if (((void*)0) == timer) {
        ...
        (INTCONbits.TMR0IE = 1);
        (INTCONbits.TMR0IF = 0);
# 57 "MCAL_Layer/Timers/hal_timer0.c"
        (INTCONbits.GIE = 1);
        (INTCONbits.PEIE = 1);
...
# 96 "MCAL_Layer/Timers/hal_timer0.c"
STD_ReturnType timer0_write_value(const timer0_t *timer, uint16 value) {
    STD_ReturnType ret = (STD_ReturnType)0x00;
    if (((void*)0) == timer) {
        ret = (STD_ReturnType)0x00;
...
# 115 "MCAL_Layer/Timers/hal_timer0.c"
STD_ReturnType timer0_read_value(const timer0_t *timer, uint16 *value) {
    STD_ReturnType ret = (STD_ReturnType)0x00;
    uint8 tmr0h, tmr0l;
...
static STD_ReturnType Timer0_prescaler_config(const timer0_t *timer) {
    ...
    if (((void*)0) == timer) {
        ...
        if (timer->prescaler_enable == 0) {
            (T0CONbits.PSA = 0);
            T0CONbits.T0PS = timer->prescaler_value;
        } else if (timer->prescaler_enable == 1) {
            (T0CONbits.PSA = 1);
        } else {
        }
    }
...
static STD_ReturnType Timer0_mode_config(const timer0_t *timer) {
    ...
    if (((void*)0) == timer) {
        ...
        if (timer->timer0_mode == 0) {
            (T0CONbits.T0CS = 0);
        } else if (timer->timer0_mode == 1) {
            (T0CONbits.T0CS = 1);
            if (timer->timer0_source_edge == 0) {
                (T0CONbits.T0SE = 0);
            } else if (timer->timer0_source_edge == 1) {
                (T0CONbits.T0SE = 1);
            } else {
            }
        } else {
...
static STD_ReturnType Timer0_reg_config(const timer0_t *timer) {
    ...
    if (((void*)0) == timer) {
        ...
        if (timer->timer0_reg == 1) {
            (T0CONbits.T08BIT = 1);
        } else if (timer->timer0_reg == 0) {
            (T0CONbits.T08BIT = 0);
        } else {
        }
    }
